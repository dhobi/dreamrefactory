 ...
 *
 * Plus, into the playthrough suite when tests/playthrough/nav/ exists:
 *   tests/playthrough/nav/shipgraph.gen.ts   the travel graph as something WALKABLE — each
...
import { sniffScript, Token } from "../src/df/script";
import { parseScript } from "../src/engine/parser";
import { CallExpr, Expr, Script, Stmt } from "../src/engine/ast";
...
// ---------------------------------------------------------------------------
// The navigation graph — tests/playthrough/nav/shipgraph.gen.ts
// ---------------------------------------------------------------------------
/**
 ...
 * plan a route — see tests/playthrough/nav/navigator.ts.
 */
interface NavTrip {
  from: string;
  to: string;
...
if (existsSync(navDir)) {
  ...
  writeFileSync(
    ...
    `/* GENERATED by tools/flowmap.ts — do not edit.\n` +
      ` * ${navTrips.length} set-to-set trips, ${usable.length} of them fully understood.\n` +
      ` * Regenerate: npx tsx tools/flowmap.ts gamefiles out/\n */\n` +
      `import type { ShipTrip } from "./shipgraph";\n\n` +
      `export const SHIP_TRIPS: ShipTrip[] = [\n${lines.join("\n")}\n];\n`,
  );
  console.log(`  nav graph: ${usable.length}/${navTrips.length} trips walkable -> tests/playthrough/nav/shipgraph.gen.ts`);
...
md.push("");
md.push(
  "_Generated by `tools/flowmap.ts` directly from the decoded scripts — no additional TI.EXE reverse engineering. " +
