...
const seenBasenames = new Set<string>(); // some files (BOOTFILE) exist in >1 dir
for (const path of walk(rootDir)) {
  ...
  for (let i = 0; i < file.containers.length; i++) {
    ...
    scriptsParsed++;
    parsed.push({ file: name, container: i, script });
...
function emitFromStmts(
  stmts: Stmt[],
  guards: string[],
  loc: { file: string; container: number; handler: string },
): void {
  for (const s of stmts) {
    switch (s.t) {
      case "assign":
...
for (const { file, container, script } of parsed) {
  for (const [handler, code] of script.codes)
    emitFromStmts(code.body, [], { file, container, handler });
  if (script.topLevel.length) emitFromStmts(script.topLevel, [], { file, container, handler: "<top>" });
}
  ...
  <script>
    ...
    function tripHtml(t, dir){
      ...
      return "<div class='trip'><div class='to'>"+dir+" "+esc(t.op)+"("+esc(sv)+")</div>"+
        g + "<div class='meta'>from "+esc(loc)+"</div></div>";
    }
    function showNode(n){
...
console.log(`scene graph:       ${graphNodes.length} nodes, ${graphEdges.length} edges`);
