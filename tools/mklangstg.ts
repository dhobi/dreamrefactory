 ...
 * the scripts — compiled to real token streams by src/df/script-asm.ts, so the
...
import { decodeScript } from "../src/df/script";
import { compileScript } from "../src/df/script-asm";
import { buildStgBytes } from "../src/df/stg-build";
...
import { Canvas, canvas, drawText, drawTextCentered, fillRect, strokeRect, textWidth } from "./pixelart";
 ...
 *
 * The colours are the website's, from src/theme.css — abyss for the water, hull
...
function globeField(): Float32Array {
  const png = decodePNG(new Uint8Array(readFileSync(join("public", "globe.png"))));
