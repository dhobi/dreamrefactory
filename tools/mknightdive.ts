 ...
 *
 * Two halves, and they are the two halves a MOV can be (docs/formats/mov.md):
 *
 ...
 *    src/nightdive.ts, which reads it back.
 *
...
import { buildMovBytes, MovBuildFrame, MovBuildSegment } from "../src/df/mov-build";
import { readMovFile } from "../src/df/mov";
import { SCREEN_H, SCREEN_W } from "../src/screen";
import { decodeGif, GifFrame, GifImage } from "./gif";
import { Canvas, drawTextCentered, fillRect, strokeRect } from "./pixelart";
...
function filmSegment(gif: GifImage): MovBuildSegment & { seconds: number; logoFrame: Uint8Array } {
  ...
  // (docs/formats/image-codec.md), so the instinct is that fewer colours mean
 ...
 * animation the repository does not ship (tests/auto/nightdive.ts).
 */
...
// (same shape as tools/mklangstg.ts)
