...
import { readSetFileV1 } from "../src/df/set-v1";
import { readSetFileAsV4 } from "../src/df/set-v1-to-v4";
import { readContainerFile } from "../src/df/container";
import { decodeFrame, FrameBuffer } from "../src/df/image";
import { decodeAudioContainer } from "../src/df/audio";
import { RIGHTTURNS, LEFTTURNS } from "../src/df/set";

const DIR = process.argv[2] ?? "gamefiles/dust/dustcd/DATA";
if (!existsSync(DIR)) {
  console.error(`no such directory: ${DIR}\nusage: npx tsx tools/dustsets.ts [dir]`);
  process.exit(1);
}
 ...
 * past. See COMPASS in src/df/set-v1-to-v4.ts.
 */
let compass = 0;
for (const f of sets) {
  let set;
...
// ---------------------------------------------------------------------------
{
  const { readStarPath } = await import("../src/df/set");
...
// ---------------------------------------------------------------------------
{
  const { sniffScript, scriptToText } = await import("../src/df/script");
  const { GameHost } = await import("../src/host");
  const { NullAudioSink } = await import("../src/engine/audio");
...
// ---------------------------------------------------------------------------
{
  const { readPupFile, readAnimLogic } = await import("../src/df/pup");
  const { decodeShpFrame } = await import("../src/df/shp");
...
// ---------------------------------------------------------------------------
const { readSndFile } = await import("../src/df/snd");
const { readMovFileV1 } = await import("../src/df/mov-v1");
const sndDir = DIR;
