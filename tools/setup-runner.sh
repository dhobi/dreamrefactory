...
[ -n "$DUST_GAMEFILES" ] || die "give Dust's rip too, with --dust:
  setup-runner.sh /srv/taoot/gamefiles --dust /srv/dust/gamefiles
It was a 'dust/' subdirectory of Titanic's tree until each game got its own root."
...
# that far.
for _d in "$GAMEFILES" "$DUST_GAMEFILES"; do
...
set_env DUST_GAMEFILES "$DUST_GAMEFILES"
