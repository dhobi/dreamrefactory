 ...
 * writing is writeContainerFile/encodeShpFrame from src/df, so an untouched
 * load exports the same structure it read (see tests/auto/pup-editor.ts).
 */
...
import { DEFAULT_ENCODING, DfEncoding } from "../src/df/text";
...
function loadPup(bytes: Uint8Array, name: string): void {
  ...
  try {
    ...
    log(t("common.notReadable", { ext: ".pup", message: (e as Error).message }));
    return;
  }
  pup = parsed;
  fileName = name;
...
async function initServerPups(): Promise<void> {
  ...
  const pups = paths.filter((p) => p.toLowerCase().endsWith(".pup")).sort();
  ...
  for (const p of pups) {
    ...
    b.addEventListener("click", async () => {
      ...
      loadPup(new Uint8Array(await r.arrayBuffer()), base);
    });
    row.appendChild(b);
  }
...
function poseForLine(ident: string | null): PupAnimFrame | null {
  ...
  // where the background plate sits (see src/df/pup.ts)
