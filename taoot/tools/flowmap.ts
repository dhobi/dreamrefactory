 ...
 *
 *   npx tsx taoot/tools/flowmap.ts                    # the `en` tree
 *   TAOOT_LANG=de npx tsx taoot/tools/flowmap.ts      # or another one
 *   npx tsx taoot/tools/flowmap.ts <rip> <outDir>     # or say where outright
 *
 * The output is committed, so it reads ONE tree by default — see {@link
 * defaultRoot}.
 */
...
import { activeLanguage, gamefilesRoot } from "./gamefiles";
 ...
 * interactive map is published into `docs/public/` — so it has to be a function
 * of the GAME and not of which editions the person running it happens to have
 * installed. Handed the whole tree it walks all of them, and while scripts are
 * deduplicated by content, the trees are not all the same game: `demo` is a
 * different CUT and contributes trips the full game does not have. Measured on
 * an install with seven trees, that is 326 trips against `en`'s 271 — a diff
 * that looks like the graph changed when all that changed is who ran it.
 *
 * So: {@link activeLanguage}, which is `TAOOT_LANG` when set and `en` when not.
 * Pass a path to override, and the run prints which tree it read.
 */
function defaultRoot(): string {
  const root = gamefilesRoot();
  const lang = activeLanguage(root);
  return lang ? join(root, lang) : root;
}

const [, , rootDir = defaultRoot(), outDir = "out"] = process.argv;
...
if (existsSync(navDir)) {
  ...
  writeFileSync(
    ...
    `/* GENERATED by taoot/tools/flowmap.ts — do not edit.\n` +
      ...
      ` * Read from the ${basename(rootDir)} tree.\n` +
