 ...
 *
 *   npx tsx taoot/tools/dumpaudio.ts gamefiles/en/titanic1/data/bedrad1.trk out/
 *   npx tsx taoot/tools/dumpaudio.ts --find doorlocked        # scan all banks
 */
...
import { gamefiles, gamefilePath } from "./gamefiles";
...
if (args[0] === "--find") {
  ...
  // sweep every bank the data carries, on both discs and in the per-room folders
  for (const f of gamefiles().names(/\.(trk|sfx|11k)$/)) {
    try {
      const file = readContainerFile(new Uint8Array(readFileSync(gamefilePath(f))));
...
if (bank.loopChunks.length) {
  const parts = bank.loopChunks.map((loc) => decodeAudioContainer(file.containers[loc].data));
...
for (const [key, ref] of bank.singles) {
  const a = decodeAudioContainer(file.containers[ref.containerLoc].data);
