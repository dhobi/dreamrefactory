...
import { encodingOf } from "../src/languages";
import { GameSession } from "@dreamfactory/engine/runtime/session";
import { AudioSink, NullAudioSink } from "@dreamfactory/engine/runtime/audio";
...
/**
 * A session under the REAL host (engine/src/web/host.ts) — the same set activation, the
...
export async function newHost<S extends AudioSink = NullAudioSink>(
  ...
  // themselves on exactly that (taoot/tests/auto/sound-channels.ts). Those pass their
  ...
  const host = new GameHost(
    {
      ...
      // and which code page that is — the host asks the source now rather than
      // looking it up in one game's table itself
      textEncoding: () => encodingOf(activeLanguage(root, opts.edition) ?? ""),
