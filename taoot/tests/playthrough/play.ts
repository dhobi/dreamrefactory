...
import { snapshotSave } from "@dreamfactory/engine/runtime/saveload";
...
/** the sinking, as a seed — any fixed number would do (engine/src/runtime/rng.ts) */
 ...
 * browser seeds these into IndexedDB (taoot/src/save-seed.ts); headless we read one
...
export async function newPlaythrough(): Promise<Playthrough> {
  ...
  // that this file and taoot/tests/browser/playthrough.ts emit the SAME text and a diff
