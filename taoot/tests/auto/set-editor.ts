 ...
 * engine/src/df/set.ts, so it runs without gamefiles/ — the editor must round-trip
...
import { FrameBuffer, decodeFrame, encodeFrame, encodeZLayer } from "@dreamfactory/engine/df/image";
import { IN_MOTION, STANDPOINT, buildSetFile } from "@dreamfactory/engine/df/set-build";
import { compileScript } from "@dreamfactory/engine/df/script-asm";
import { sniffScript } from "@dreamfactory/engine/df/script";
import { LEFTTURNS, RIGHTTURNS, SetFile, readSetFile, readStarPath, roadsAt, turnRing } from "@dreamfactory/engine/df/set";
import {
  OBJECT_ID_FIELD,
  SET_NAME_FIELD,
  patchActor,
