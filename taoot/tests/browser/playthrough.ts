...
import { isCoinFlip, isHarnessPaced } from "@dreamfactory/engine/runtime/masks";
...
/** must match taoot/tests/playthrough/playthrough.ts — the trace is only comparable per-seed */
const SEED = 19120415;
 ...
 * (engine/src/runtime/session.ts, the `handitem` block) and every use has one shape — a
 ...
 * (taoot/src/nightdive.ts `introPlaysFor`).
 */
...
async function playSegment1(page: Page): Promise<unknown[]> {
  ...
  // -- 1-2. cold boot --------------------------------------------------------
  // Nothing to click: the page boots itself now (taoot/src/main.ts). There WAS a
  ...
  for (let turn = 0; turn < 24 && clicked.size < BEDSIT_OBJECTS.length; turn++) {
    ...
    if (clicked.size < BEDSIT_OBJECTS.length) {
      ...
      // silently (the fade infidelity, written up at `pressNav` in engine/src/web/viewer.ts).
...
async function loadCheckpoint(page: Page, name: string): Promise<boolean> {
  ...
  //
  // The page boots itself now (taoot/src/main.ts, "Straight into the game"), so a fresh
...
/** what a flat's "leave" region is called, commonest first — as taoot/tests/playthrough/playthrough.ts */
...
async function main(): Promise<void> {
  ...
  // rolling buffer (taoot/src/log-buffer.ts), so past 5000 lines the oldest go and a
  ...
  // was a frame that would have looked the same (taoot/tests/browser/repaint.ts). The
  ...
  // run's (taoot/tests/browser/proptrace.ts).
