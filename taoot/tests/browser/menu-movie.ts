 ...
 *
 * Uses Playwright's managed Chromium (`npx playwright install chromium`), so no
 * system browser path is hardcoded. Point it at a running dev server with
 * APP_URL (defaults to Vite's http://localhost:5173/). Screenshots go to
 * SHOT_DIR (defaults to the cwd). Run with: `npx tsx taoot/tests/browser/menu-movie.ts`. */
...
// `out` rather than the repo root: these four are evidence for a human to look
// at, not fixtures, and dropping them next to package.json means every run
// leaves four untracked files behind. out/ is gitignored.
...
const main = async () => {
  ...
  // The page boots itself into the game now, so there is no set list to pick
  // from — this asks the host for the room directly, which is what the button
  // did anyway. Waiting for `dbg` first: it appears with the module, before the
  // manifest that `loadServerSet` needs has been read.
  await page.waitForFunction(() => !!(window as any).dbg?.host, null, { timeout: 15000 });
  await page.waitForFunction(
    () => ((window as any).dbg.host.files?.serverSetNames?.() ?? []).length > 0,
    null,
    { timeout: 15000 },
  );
  await page.evaluate(() => (window as any).dbg.host.loadServerSet("c73.set"));
