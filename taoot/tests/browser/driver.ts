 ...
 *
 * The twin of taoot/tests/playthrough/driver.ts, and the reason a route can be watched
 ...
 * watch?") are synchronous by design (see taoot/tests/playthrough/story.ts). So the
...
export function playUrl(base = process.env.APP_URL): URL {
  const root = base ?? "http://localhost:5175/";
...
export async function browserDriver(page: Page, opts: BrowserDriverOptions = {}): Promise<BrowserDriver> {
  ...
  const driver: BrowserDriver = {
    ...
    clickThing: async (name) => {
      ...
      // (taoot/tests/playthrough/driver.ts): a pixel picked before the cast stops moving
