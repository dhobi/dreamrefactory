 ...
 * band. That is what {@link WirelessDriver.propValue} exists for.
 */
/**
 * The sliver of a driver the wireless set actually needs.
 *
 ...
 * structural, so the playthrough suite's `NavDriver` satisfies it unchanged and
 * nothing on that side had to move — the same arrangement `DialDriver` is under
 * in nav/dials.ts, and for the same reason.
 */
export interface WirelessDriver {
  /** the flat that is up, or null for a room */
  inFlat(): string | null;
  /** click something by name, through the engine's own hit test */
  clickThing(name: string): Promise<boolean>;
  /** take hold of a prop and move the cursor, one step per call, until `next` says stop */
  dragProp(name: string, next: (from: { x: number; y: number }) => { x: number; y: number } | null): Promise<boolean>;
  propOwner(name: string): string;
  /** the needle's FREQUENCY — see the header on why this and not `propDeg` */
...
export async function closePanel(d: WirelessDriver): Promise<WirelessResult> {
  const from = d.inFlat();
 ...
 */
export async function setSender(d: WirelessDriver, want: "on" | "off"): Promise<WirelessResult> {
 ...
 */
export async function tuneTo(d: WirelessDriver, band: { lo: number; hi: number }): Promise<WirelessResult> {
 ...
 */
export async function switchToTransmit(d: WirelessDriver): Promise<WirelessResult> {
  for (const step of [
