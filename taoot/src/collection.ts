 ...
 * (taoot/src/home.ts), then the 3D box, the disc it sits beside, and — below both —
 * the printed booklet, which is taoot/src/booklet.ts's and reached from here only
 ...
 * ## Which edition
 *
 * The site's own edition axis (taoot/src/editions.ts), which the play page and the
 ...
 *
 * The endonyms on the buttons are read off `taoot/src/languages.ts` rather than typed
...
/** the release's own name, in its own language — the game's own title, so it
    is never run through the catalogue (see site/src/locales/en.ts's `collection`
...
/** every box held a printed booklet; the German one is the only one the old
    site ever scanned, which is why taoot/src/booklet.ts has an answer for `null` */
...
// The same row the play page and the editors carry (taoot/src/editions.ts), over this
