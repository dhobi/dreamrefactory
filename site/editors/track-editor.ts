 ...
 * decodeAudioContainer); writing is the patches in engine/src/df/banks.ts plus
...
/**
 * A Dust `.SND`, shown through the tables this page already draws.
 *
 * Dust spells an audio bank `.SND` and stores it differently — one table holding
 * every sound, where a v4 TRK has a loop table and a one-shot table in separate
 * containers. The engine reads it already (`snd.ts`, for the runtime), and its
 * `chunks` are `BankChunk` values on purpose: "the same shape BankChunk has on
 * the v4 side, so a consumer that resolves a name to a container does not care
 * which engine wrote the bank."
 *
 * So this reshapes rather than parses. The theme is the trailing run of
 * consecutively numbered chunks, which is what `sndLoopChunks` finds and what
 * the game plays.
 *
 * It differs from `readV1Bank` in banks.ts in ONE way, deliberately: that
 * function keys EVERY chunk as a one-shot, because a script may ask for any of
 * them by name and lookup does not care that ten of them are also the theme.
 * This page is a display, and counting those ten twice made a 25-sound bank
 * report "35 chunks total". So the run is taken out of the one-shots here, which
 * also makes a v1 bank read the way a v4 one does: a theme, and the sounds
 * beside it.
 *
 * The zeros are honest: there are no v4 table containers to point at and nothing
 * here is patchable, which is why a `.SND` opens READ-ONLY.
 */
function bankTablesFromSnd(snd: SndFile): BankTables {
  const loop = sndLoopChunks(snd);
  const inLoop = new Set(loop);
  const byLoc = new Map(snd.chunks.map((c) => [c.containerLoc, c]));
  const loopRecords = loop
    .map((loc) => byLoc.get(loc))
    .filter((c): c is BankChunk => c !== undefined);
  return {
    trackName: snd.refName,
    trackNameLimit: 0,
    loopTable: 0,
    oneShotTable: 0,
    // the run in file order; a v1 bank has no order table because the names ARE
    // the order
    loopOrder: loopRecords.map((_, i) => i + 1),
    loopRecords,
    singles: snd.chunks.filter((c) => !inLoop.has(c.containerLoc)),
  };
}
 ...
 * patch has just changed it, and a v1 bank has to be re-derived from the SAME
 * place it came from. Reading it back with `readBankTables` instead gave a
 * garbage track name and no chunks at all: that reader looks for v4 tables in a
 * file that has none.
 */
...
/**
 * Refuse a mutation on a reshaped v1 bank, and say so.
 *
 * Not decoration: the patch helpers write at v4 offsets — `patchLoopOrder` edits
 * the loop table in container 1 — and in a `.SND` container 1 is a SOUND. So a
 * reorder on a Dust bank would overwrite audio in the loaded file, and the WAV
 * beside it would come out as noise. Every mutation this page has goes through
...
/** peak-to-peak waveform, the shape taoot/tools/dumpaudio.ts draws as a PNG */
