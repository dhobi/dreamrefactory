/**
 * The page text (site/src/locales/), and the contract that makes it safe to keep
 * English in two places at once.
 *
 * The eleven pages carry their English inline so that an English reader needs no
 * JavaScript and `git diff` still shows the sentence that changed; site/src/locales/en.ts
...
import { UI_LANGUAGES, DEFAULT_UI_LANGUAGE } from "../src/ui-languages";

// the repository root: the pages this reads belong to two different packages
const ROOT = fileURLToPath(new URL("../..", import.meta.url));

/** every page carrying a translated top bar, across the three builds */
const PAGES = [
  ...
  "site/editors/puppets.html",
  "site/editors/casts.html",
  "site/editors/sets.html",
  "site/editors/shops.html",
  "site/editors/stages.html",
  "site/editors/movies.html",
  "site/editors/tracks.html",
];
 ...
 * how these values reach the page (site/src/locales/index.ts), so this list is the
...
const LOCALES = join(ROOT, "site/src/locales");

/** the `site/src/locales/<code>/` directories, which is what a translation IS */
...
test("a language directory and its registration arrive together", () => {
  ...
  for (const code of onDisk) {
    expect(UI_LANGUAGES.map((l) => l.code), `${code} is not one of the six the chrome is written in`).toContain(code);
    expect(code, "English is the source, not a translation").not.toBe(DEFAULT_UI_LANGUAGE);
...
test("all five shipped languages are present", () => {
  // UI_LANGUAGES is the six the CHROME is written in; the pages are expected to
  ...
  expect(translationDirs()).toEqual(
    UI_LANGUAGES.map((l) => l.code).filter((c) => c !== DEFAULT_UI_LANGUAGE).sort(),
  );
});
...
// (`.editionPicker` in site/src/theme.css, `#help .bar` here), which is a good sign
// this wants a test rather than a third comment.
test("elements toggled with .hidden survive their own display rule", () => {
  const html = readFileSync(join(ROOT, "taoot/play/index.html"), "utf8");
  const style = [...html.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/g)].map((m) => m[1]).join("\n");
  const main = readFileSync(join(ROOT, "taoot/src/main.ts"), "utf8");
