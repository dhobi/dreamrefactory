...
export interface Headless {
  ...
  /** step until the player has the game again: the room on screen and nothing running */
...
export async function headless(): Promise<Headless> {
  ...
  const until = async (done: () => boolean, what: string, max = 40_000): Promise<number> => {
    ...
    if (done()) return max;
    fail(`stuck waiting for ${what} (${max} passes, t=${Math.round(clock / 1000)}s, ${owner()} at ${room()}/${node()})`);
  };
  ...
  const settle = async (what: string, max?: number): Promise<number> => {
    ...
    await frame(3);
    return until(
      () => owner() === "world" && !session.maze?.walk && host.director.quiescent,
      `${what} to settle`,
      max,
    );
  };
  ...
  return { host, session, logs, disc: () => disc, frame, until, settle, owner, room, node, click, key };
}
