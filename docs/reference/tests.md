...
| [`speedrun-loads.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tests/auto/speedrun-loads.ts) | the **load remover** — a run's clock stops while the game is coming over a link ([#251](https://github.com/dhobi/dreamrefactory/issues/251)). Every room in this port is a fetch, so an identical sheet over a cold cache and a warm one is minutes apart with no gesture changed, and a wall clock therefore measures the link as much as the route. Three seams, none of them a browser: the fetcher reports the wire to as many watchers as ask (it was one assignable slot, which the busy mark on the canvas had), `LoadClock` turns those edges into a total, and the run loop subtracts it from every leg. The arithmetic is the part worth getting wrong and is pinned with time as a variable — a cache hit and a `localhost` fetch are disk reads and cost the route nothing (#369, which is what made the workbench faster than the runner on one route), six downloads in the air are ONE wait and not six (adding their durations up would remove more than the run spent), the fetch still in the air counts (a reading that ignored it would tick through a 37 MB film and then jump backwards), a cache hit is not a load, a fetch spanning two actions is charged to each in part and to the pair once, and `time + load` comes back to the wall clock exactly |
| [`input-log.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tests/auto/input-log.ts) | the **input log** — what the player pressed, beside what the game said about it ([#178](https://github.com/dhobi/dreamrefactory/issues/178)). Tested as WORDING, with no browser and no engine, because wording is what it is: the module asks the page four questions (the clock, the location, the gate, and a hit test) and decides what to say. Most of the file is about the gestures that did NOTHING, since a line claiming otherwise would send a reader looking for the bug somewhere else — a press the engine threw away must not read as one that worked, a press filed behind a camera move must say so, and the Nightdive intro's ESC ([#171](https://github.com/dhobi/dreamrefactory/issues/171)) must not come out as "nothing changed" when it just skipped a film. Also that a log switched off costs the engine no hit test, and that a slow gesture's cause line lands before the engine lines it caused |
