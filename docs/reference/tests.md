...
|----------|-----------|---------|------------|
| **automatic** | `taoot/tests/auto/` | `npm test` | scenario + unit suites that jump to a state and probe it — 456 tests, run on every commit |
...
npx tsx taoot/tools/navdump.ts gamefiles/en/titanic2/DATA/b59.set out/   # navigation dump (PNG per step)
```
