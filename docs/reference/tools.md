...
| [`play.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tests/browser/play.ts) | `npm run play -w dust -- <save>` — opens Dust in a real window at any rung of the disc's own playthrough, using `dbg.loadSave` (the panel's LOAD lever minus the modal it blocks on). `--list` prints the saves in the order they were made. Asserts nothing and never exits on its own; it is the companion to [the walkthrough](../dust/walkthrough.md), whose every step names the save that ends it. It says what it is waiting for BEFORE the three-minute boot and logs the standpoint as you walk, because a window still booting and one that has hung look identical |
| [`actionframes.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/actionframes.ts) | `npx tsx dust/tools/actionframes.ts [--verbose]` — asks every movie on the disc whether the frame its header calls **action frame 1** can be arrived at, under a 1-based reading of that header and under a 0-based one. `actionframe (n)` is how a script asks "did the film get as far as the bit that matters?", and it decides whether you keep what the film was about — the postcards in the Mayor's study, the bullets at the stagecoach depot. **The disc is inconsistent**: twelve segments name an action frame, all twelve name a reachable frame read 0-based, eight do read 1-based, and the four that fail (`abe.mov`, `safebox.mov`, `salgun.mov`, `wellgun.mov`) are unreachable in a way that stops the trade the shipped saves show completing — while `MAYBED.MOV` positively supports 1-based, its bed-click hotspot landing exactly on the frame that reading names. The question is open; the tool prints the evidence rather than picking a side |
| [`flatprops.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/flatprops.ts) | `npx tsx dust/tools/flatprops.ts <file.flt> [--scripts]` — the flats in a stage file, and on each one the **click regions**: a rectangle, a name, and the container index of the script it runs, with a one-line summary of what that script does. A flat is a full-screen layer where the arrow keys do nothing and everything is a click, and its scripts say `gotoflat (4)` without ever saying where on the screen that is — so a route across one is guesswork until this is read — random clicks through `getcards.mov` keep taking the walk-away branch, and clicks spread over `hotplate.flt` miss the picture that ends breakfast |
| [`rtdthread.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/rtdthread.ts) | `npx tsx dust/tools/rtdthread.ts [--spine] [--all] [--md] [--rung <name>]` — reads every `.rtd` beside the Dust disc, orders them by the service-pass counter, and diffs each against the one before it. They are not examples: all but one are **a single continuous session** written by the shipped `DF.EXE`, from the town gate to the ending ([the golden thread](../dust/thread.md)). It proves the lineage rather than assuming it — frame order is not lineage, so the walk tracks the running maximum `day` and names what goes backwards — and it filters the engine's own counters through a named list with a reason each rather than a heuristic. A prop's `owner` field going `"none"` → `"stranger"` is "you picked it up", which is why [a walkthrough](../dust/walkthrough.md) falls out of it |
| [`dustsets.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/dustsets.ts) | `npx tsx dust/tools/dustsets.ts` — reads every SET on the Dust CD through the v1 reader and says what came out. The last line is the one that matters: `warnings` is everything the reader had to assume and could not confirm, the evidence the [v1 model](../engine/formats/set.md#dreamfactory-1-dust) rests on |
...
**The language matters as much as `BIN` over `BINX`.** Every localised disc
ships its own build — `de/BIN` is 463,872 bytes against `en/BIN`'s 461,312 — so
a hardcoded VA lands somewhere that is not code in any of them. Against the
wrong build `disasmcmd.mts` reads garbage and reports *"no handler found in
dispatch tables"*, which is indistinguishable from a command nobody has located
yet; ranking candidates on the path alone puts `de/` first alphabetically, so
the sort prefers `en/` explicitly. Measured: `disasmcmd calcvectx` yields 35
instructions against `en/BIN` and nothing at all against the other eleven
candidates.
...
| Tool | What it does |
|------|--------------|
| [`mapjumps.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/mapjumps.ts) | `npx tsx taoot/tools/mapjumps.ts` — reads `MAP.STG`, decodes each deck plan's click regions and pulls the `jumpbaby(...)` out of the script behind each one, emitting `taoot/tests/playthrough/nav/mapjumps.gen.ts`. It reads the `if` above the jump too, which decides what the deck map does: [15 of the 32 red areas do nothing in a shipped game](../taoot/verification.md#taking-the-deck-map) |
| [`devmenu.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/devmenu.ts) | `npx tsx taoot/tools/devmenu.ts` — walks `TI.EXE`'s PE resource tree to its `RT_MENU` resource, parses the `MENUITEMTEMPLATE` records and emits `taoot/src/devmode/menu.gen.ts`: the developer menu bar `menuvisible (debugging)` put up, with the Win32 command ids and `Ctrl+X` accelerators the resource gives them. The menu is in the executable and nowhere in the game data — the corpus has no menu-defining command at all — so it is read from there rather than written down. See **[Developer mode](../taoot/devmode.md)** |
| [`devcensus.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/devcensus.ts) | `npx tsx taoot/tools/devcensus.ts` — sweeps every script-bearing file for reads of `debugging` and calls to `shiftkey`/`optionkey`/`commandkey`, classifies each branch by the call it makes, and writes **[the census](../taoot/devmode-census.md)**. Generated rather than kept by hand because the answer is in the hundreds and moves with the decompiler |
| [`mapareas.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/mapareas.ts) | `npx tsx taoot/tools/mapareas.ts` — reads all 32 of the deck map's jump areas out of `MAP.STG` with their RECTANGLES, and emits `taoot/src/devmode/areas.gen.ts` for the overlay [developer mode](../taoot/devmode.md) draws over the plans. The near neighbour of `mapjumps.ts` and deliberately not the same table: that one emits the 17 a player can use and drops the rest, and the point of the overlay is to show where the rest are |
...
| Tool | What it does |
|------|--------------|
| [`mklangstg.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/mklangstg.ts) | `npm run mklang -w taoot` — builds `public/lang.stg`, the **language chooser**, as a real two-flat STG: its own palette, art, click regions and compiled scripts. Needs no `gamefiles/` and reads nothing from the game, which is why the result ships in the repository. Pass a directory to write somewhere else. See [writing a stage](../engine/formats/stg.md#writing-a-stage) and [Languages & the chooser](../taoot/languages.md) |
...
file you give them, so they have their own section:
**[the browser editors](../editors/README.md)**.
