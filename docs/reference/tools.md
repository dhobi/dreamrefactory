...
| [`actionframes.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/actionframes.ts) | `npx tsx dust/tools/actionframes.ts [--verbose]` — asks every movie on the disc whether the frame its header calls **action frame 1** can be arrived at, under a 1-based reading of that header and under a 0-based one. `actionframe (n)` is how a script asks "did the film get as far as the bit that matters?", and it decides whether you keep what the film was about — the postcards in the Mayor's study, the bullets at the stagecoach depot. **The disc does not agree with itself**: twelve segments name an action frame, all twelve name a reachable frame read 0-based, eight do read 1-based, and the four that fail (`abe.mov`, `safebox.mov`, `salgun.mov`, `wellgun.mov`) are unreachable in a way that stops the trade the shipped saves show completing — while `MAYBED.MOV` positively supports 1-based, its bed-click hotspot landing exactly on the frame that reading names. Unsettled, deliberately: this prints the evidence rather than picking a side |
| [`flatprops.ts`](https://github.com/dhobi/dreamrefactory/blob/master/dust/tools/flatprops.ts) | `npx tsx dust/tools/flatprops.ts <file.flt> [--scripts]` — the flats in a stage file, and on each one the **click regions**: a rectangle, a name, and the container index of the script it runs, with a one-line summary of what that script does. A flat is a full-screen layer where the arrow keys do nothing and everything is a click, and its scripts say `gotoflat (4)` without ever saying where on the screen that is — so a route across one is guesswork until this is read. It exists because guessing failed twice: thirty clicks through `getcards.mov` took the walk-away branch every time, and forty spread over `hotplate.flt` never found the picture that ends breakfast. The answer was in the file both times |
...
| [`mknightdive.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/mknightdive.ts) | `npm run mknightdive -w taoot -- heading.gif` — builds `nightdive.mov`, the intro film and the ownership question after it, out of an animated GIF. Two halves, and they are the two halves a MOV can be. The film is generated at build time rather than committed ([why](deploy.md)) |
