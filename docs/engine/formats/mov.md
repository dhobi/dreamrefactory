...
DFET decodes the frames and audio; the *interactive* behaviour — what a click
actually does — is recovered from `TI.EXE`'s movie loop.
...
A MOV is not a linear video but a **flip-book where each page can have buttons
on it**. The player sits on a frame; the frame may just play and advance, or it
may **wait for you to click**. Where a click takes you is defined per-region,
per-frame. That **state machine** is why one format can be both a passive
cutscene and an interactive close-up.
...
**A MOV file is not one film but a linked list of them.** Header `+0x2c` holds
the container index of the **next segment's header** (0 = last), and a segment
is the whole structure below over again — its own palette, frame table, audio
...
The demo's `TRAILER.MOV` is 13 segments (698 frames — the frame table at
container 0 alone holds 139, which is 9.8 s of picture under 92 s of
narration). `TOUR.MOV` is 20, one per narrated slide. The full game does it
too: `SINK1..6`, `DEBRIS`, `LOGO` and above all `LEAVE.MOV` — **10 segments,
1628 frames, the entire sinking montage**, 37.5 MB.

### How the reader and the write path are shaped around it

This decides whether an edit lands on the frame you were looking at or on an
unrelated one:
...
- `readMovFile` walks the chain and returns a `MovFile`, which **IS its first
  ...
  single-segment movie reads as a plain file. A self-referencing `+0x2c` cannot
  loop the reader (it keeps a seen-set; the engine would loop the *player*
  instead).
...
- So **every patch is addressed to a `MovSegment`, not to the file**:
  ...
  `patchKeySkips(mov, …)` the first segment's. Container 0 is only the *first*
  segment's header; writing a later segment's edit there renames an unrelated
  frame at the same record offset.
...
A MOV holds two kinds of bulk at once: the frames and the soundtrack. The
header, the frame table and the per-frame logic containers are the handful of
small blocks at the start; everything after them is a picture or a slice of
sound.
...
27.1 s in all. Without the field only the first chunk plays and the frame holds
for ever.

The watch frames also show what the clearing rule is for: every one of them
carries an exit region and a whole-screen region to `endwatch`, whose entry sound
is `sil` — a second of **silence** that both takes the channel from the line and
...
frame read as pointing there would take its action type from the fourCC's low
half.) And a container **shorter than `0x42`** is not trusted for any of the
fields above; one shorter than `1094` simply has no region table.
...
constant `0x442` = 1090 locates the routine in the disassembly)*, then
...
**A movie's parked REGIONS chain as well as its frames.** Example: the car keys
in the Purser's office. `docarkeys()` gives them on `actionframe(1)`, and neither
`maino2.mov` nor the `man.mov` its "blackframe" leads to declares an action
frame 1 — but `maino2.mov` frame 0 parks with two type-4 regions sharing the
target "win 1", one of which chains to **`key.mov`**, which does declare action
frame 1. So the keys are a gesture: click them on the wall, click them again in
the close-up (that frame *is* the action frame), click the plaque.
...
movie's **regions**, not just its frame table.
...
- **Only the loop table plays by itself.** The engine's sole self-started
  ...
  *one-shot* chunk is an **event** sound — fired by a click or by entering a
  frame (`FAUCET.MOV`), never as music and never automatically. (In the
  single-chunk cutscenes the chunk is also frame 0's entry sound, so it sounds
  like a soundtrack; `LEAVE.MOV`'s segment 2 shows the difference, its
  smokestack crash being an entry sound, not a bed.) An
...
- Two fields that *look* meaningful are **authoring leftovers the engine never
  ...
  A field being present does not mean the engine uses it.
- The word at **`+4`** is not a field: it is the high half of the **i32 hold at
  `+2`** — the frame's authored duration in ticks (`0x44b10f` reads the full
  dword, as the demo build's movie loop shows). `TOUR.MOV`'s frame 6 being a
  hold and a backward `goto` does not hide frames 7..13: the **cue table** jumps
  playback out of that authored loop at tick 200, and the rest of the tour is
  nineteen more **segments** — see
...
| 2 | an alternate frame blit (`0x438900` rather than `0x438850`) | `BOMBHELP`, `AFTWASH`, `PORTWASH`, `STARWASH` — the port does not distinguish the two paths |
...
In the original, **no part of this is in the window proc**. That layer only
translates (VK_ESCAPE → `.` + marker) and posts to one shared event queue;
whichever modal loop is running pops from it — the main interactive loop takes
anything (`0x431c43`), a text prompt takes keys only (`0x440756`), the movie loop
takes keys and its own kinds (`0x44a16b`). A live movie is therefore the
*consumer* of the keystroke, which is both why it can abort and why the key never
reaches the script chain while a movie plays.

The port mirrors that split exactly:
...
Every movie's frames carry an authored hold. `FAUCET.MOV`'s frames, for example,
are authored at **50 ms**; its water loop repeats about three times under the
3.62 s sound, so the audio does not give the frame rate.
...
shows them, nothing paces on them; picture repetition comes from segments and
cues, not from the bed.

The **soundtrack** has twists that a naïve reading gets wrong, and they matter
for playing the audio itself:
...
- How much of the order to take is **how long the bed will be on screen, plus a
  margin** — and "on screen" has three parts.
  ...
  prediction ends before the picture does (the fly-in to C73 carrying on
  silent).
  ...
  Missing all three cuts the 1996 demo's opening — a 23-entry order holding
  156 s — to 25.18 s (one segment's prediction) under a film that runs 31.4 s,
  and the music restarts. The player takes **110%** of the measured runtime out
  of the authored order, which for `OPEN.MOV` is 33.87 s and never reaches the
  end.

  The loop is the backstop, and it applies **only when the order itself ran out**.
  Reaching the end of a bed that was cut short means the *cut* was short, and
  rewinding there plays the author's first chunk, which is never what comes next.
  Across all three discs no bed falls short of its film. The bed is halted with
  the film either way — `TI.EXE`'s own teardown does that on the no-next-segment
  path (`0x449d40`).

### A movie carries its own pacing

Read out of the demo build's engine: the timing is **in the file**, and it is per
frame, not derived from the soundtrack.
...
- **bit 0 — wait for the spoken line.** The loop will not leave the frame while the
  ...
  one of `LEAVE.MOV` (68) — the latter is why that film's last line outlives its
  picture.
- **bit 2 — do not WAIT on this frame's click regions**: honour them only if a
  ...
  **This is where their loops come from; there is no "loop" field anywhere in
  ...
  that action by falling through the zeroed region count. Without bit 2 the
  animation stops on its own first frame and advances a frame per click
  ([#172](https://github.com/dhobi/dreamrefactory/issues/172): the horses in the
  gym).
...
**Which engine.** Both builds' movie loops consult audio (through `voicedone`'s
primitive, `0x424cf0`, not `sounddone`'s). In the retail `TI.EXE` (7 Oct 1996)
the voice wait is `0x44a0e0`, called from the frame body once the frame's own
wait is over, and the sound-driven jump above polls the channel from every wait
loop. The **demo** ships its own build (`INSTALL/BIN/TI.EXE`, 18 Jul 1996,
448,512 bytes against the retail 461,312) whose same routine sits at `0x44a8e0`;
its dispatch tables are at `0x427f58` (value) and `0x43e1d0` (action). Neither
build derives a frame RATE from audio — the pacing is the authored holds.

**Why it matters.** Deriving the rate from audio plays the demo's `TRAILER.MOV` at
...
bit 3, which pulls it in. `FAUCET.MOV` would read as 145 ms/frame from its water
audio (3.62 s over 25 frames); authored, its frames are **50 ms**.

Pacing by authored holds moves **only** these goldens: `attentionspan`,
`clockcount`, `fourcount`, `jonesframe`, `ladycount`, `lastsail`, `paintframe`,
`sec` and `secframe` — counters and stock-line rotators. No story field moves,
and `min`/`clock` do not move at all.
...
Beside [TOUR4.MOV](#whats-in-a-segment) the family resemblance is plain:
same header container, same table, same one-picture-per-container rule. The
differences below are in what the records *say*, not in how the file is
arranged.
...
- **Hotspots are typed records** walked from a per-frame offset (record
  ...
  `+0x06` is not the count and not a field the movie loop touches: its bits
  amount to "first frame" and "last frame". Reading it as the count makes only
  frame 0 wait and lets every frame reached *by* a click run on to the end of
  the film (#324: the Mayor's letters, `maylett.mov`, and the hotel room's
  blinds, `hwin.mov`, each opening for a single frame and then closing). The
  count also bounds the run: runs are adjacent, each exactly its own count of
  records long, so an unbounded walk makes a frame owning none answer clicks
  with the next frame's boxes. All 520 counted runs on the disc decode cleanly
  for exactly their count; an unbounded walk over-reads 372 of them.
...
- **[`taoot/tools/mknightdive.ts`](https://github.com/dhobi/dreamrefactory/blob/master/taoot/tools/mknightdive.ts)**
  ...
  **action-frame** slots rather than through anything the page installed. Two
  things it illustrates: a region-less segment that never steps is read as a
