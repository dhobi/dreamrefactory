...
Reference implementation: [`engine/src/df/image.ts`](https://github.com/dhobi/dreamrefactory/blob/master/engine/src/df/image.ts), ported
...
- **The player can move it.** F1/F2 scale all three exponents down/up (the pair the
  ...
  a factor of 1.05. See `engine/src/web/screen-gamma.ts` for the jump table this comes out of,
...
juggling is in [`image.ts`](https://github.com/dhobi/dreamrefactory/blob/master/engine/src/df/image.ts); the takeaway is just
