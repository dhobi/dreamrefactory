...
vote. `nodeVtable` in `engine/src/df/savegame.ts`; the fixture and the regression are
...
`{total, 0, count, bbox, points}` that `readStarPath` (engine/src/df/set.ts) reads out
...
[`loadGame`/`snapshotSave`](https://github.com/dhobi/dreamrefactory/blob/master/engine/src/runtime/saveload.ts)
and the `savegame`/`opengame` builtins in
[`engine/src/runtime/builtins/savegame.ts`](https://github.com/dhobi/dreamrefactory/blob/master/engine/src/runtime/builtins/savegame.ts).
