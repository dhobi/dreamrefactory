...
| the **picture** | scrub any frame, step through, or **▶ Follow the machine** — which walks the **logic of the segment on screen** at the film's **own authored pace** (`frameHoldMs`, the same rule the game uses; see [pacing](../engine/formats/mov.md#a-movie-carries-its-own-pacing)) and stops where that walk has an answer: on a frame with regions, saying so, because that frame waits for a click |
| **▶▶ Play the film** | the other button, and a different question. This plays the **file**, the way the game plays it: every [segment](../engine/formats/mov.md#a-file-is-a-chain-of-segments) in order, each on its own palette and its own decode chain, paced by the frames' holds, with the segment's **bed** under it and the frames' **entry sounds** fired as they are entered — a segment that brings no audio keeps the bed already playing. It starts from **the frame you are on**, so scrub to a moment and press play; the scrub slider and the segment picker follow it while it runs. A frame that waits for a click **parks** the film rather than ending it (the bed goes on, a cue can still lift it out), and clicking the picture takes the click and plays on: `faucet.mov`'s tap runs, `camelsee.mov`'s horses gallop and keep galloping. A chain to **another file** (types 3/4/5) is reported and ends the run — following it would swap the file under your unexported edits |
