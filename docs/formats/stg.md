...
- **[`src/df/stg-build.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/stg-build.ts)**
  ...
  region records.
- **`encodeScript`** in
  [`src/df/script.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/script.ts),
...
a builtin, is in **[Languages & the chooser](../runtime/languages.md)**.
...
- `tools/mapjumps.ts` — reads `MAP.STG`'s regions to recover where each deck-plan
  hotspot jumps to.
