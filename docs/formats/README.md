...
They can now. Beside each `src/df/<fmt>.ts` reader sits a
`src/df/<fmt>-build.ts` writer — SET, SHP, STG, PUP, CST, MOV and the audio banks
— over one shared piece of scaffolding,
[`build.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/build.ts): a
...
[`set-patch.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/set-patch.ts)
...
[`script-asm.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/script-asm.ts),
