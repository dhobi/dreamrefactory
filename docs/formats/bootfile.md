...
[`src/engine/session.ts`](https://github.com/dhobi/taoot-web/blob/master/src/engine/session.ts) and the interpreter's
fallback resolution in [`src/engine/interp.ts`](https://github.com/dhobi/taoot-web/blob/master/src/engine/interp.ts).

## Why the boot is the "standard library"

Recall from [the scripting doc](../03-scripting-language.md) that an
...
Because events run the whole [event chain](../03-scripting-language.md#the-chain-and-how-an-event-is-consumed),
...
room opens — and they stay loaded for the **whole session**, not per-set:
