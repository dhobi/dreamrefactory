...
[`opcodes.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/opcodes.ts)
...
`encodeScript` in
[`script.ts`](https://github.com/dhobi/taoot-web/blob/master/src/df/script.ts)
...
business. `tests/auto/script-encode.ts` asserts the former, and that compiled
source parses into the handlers it declared. The first real user is the language
chooser's stage — [Languages & the chooser](../runtime/languages.md).
