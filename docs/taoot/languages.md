...
*Prerequisite: [The browser host](../engine/runtime/host.md) for the page and the file source;
[STG — stage files & the UI](../engine/formats/stg.md) for what a stage is.*
...
wrong entry does not fail, it mojibakes. So `taoot/tests/auto/text.ts` **re-derives every
...
[writing a stage](../engine/formats/stg.md#writing-a-stage) — and it runs in the
...
4. read this game's **volumes** out of its BOOTFILE and hand them to
   ...
   ([the boot plan](../engine/runtime/host.md#the-boot-plan-what-a-game-says-it-needs)) and the
...
and again in `site/src/locales/en.ts`, so that a translator has a list and the
translations have a shape. Two copies of a string is normally a bug waiting to
happen; what makes it safe here is `site/tests/locales.ts`, which parses the
