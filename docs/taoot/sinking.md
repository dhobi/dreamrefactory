...
and nudge it — including walking around.
...
rate, not a definition, and the sinking clock's odd behaviour follows from the
difference.
...
> **The game sinks the ship in the early afternoon.** `hrs = 13` is
...
> is dramatising. Nothing in the game shows the error: the watch face has
...
> `hrs = 1` (115 … 205). Either half of the day would have worked.
...
The second row is deliberate — it is in the BOOTFILE, gated on the mission:
...
- **Turning continuously** — `openscene` fires every 10 frames, and the throttle
  ...
  **instead of** it decides the rate, because a bump also does `clockcount = 0`.
  The video says on top (**2×**); the mechanism, worked through below, says
  instead of (**1×**), and 1× is what this port does. See [the model,
...
per scene while travelling. The arithmetic and the observation were derived
independently; together they are the evidence that the bump is real and that its
throttle is in frames.

For turning the two disagree. The same video shows the second hand moving about
twice a second while spinning in place, and the mechanism as written gives once.
That discrepancy is [#184](https://github.com/dhobi/dreamrefactory/issues/184),
worked through at the end of this page.
...
lives; stopping short of the boot scripts makes turning in place free in the
endgame ([#127](https://github.com/dhobi/dreamrefactory/issues/127)). The same boot
handler also rebuilds the nav arrow and the destination sign on a turn, so the
engine runs the handler rather than re-deriving those two.
`SetScripts.viewChanged()` uses the same chain scene entry does, and
...
|---|---|
| standing | 0.95 |
| turning continuously | 1.00 |
| entering a scene | +1 per scene (`secframe` advances) |

**Turning is not the video's 2×**, and the section below explains why that is a
statement about the *original* rather than about this port. A bump does not add
...
two. The original orders those two the same way this port does — its own `idle()`
draws the frame before it winds the clock — so the mechanism it runs predicts 1× as
well.

### The model, derived — and what it predicts

Two mechanisms could give 2×
...
animation. The boot library rules out the first and the disassembly the second;
the remaining model predicts what the port does.
...
three-way branch that chooses how to draw. The pass itself is 50 ms, read from the
disassembly: `0x43A940` spins until `framerate` ticks have elapsed,
...
`secframe = frame()` and `clockcount = 0` together, so both counters
...
It is unexplained. For the sum to be 2 the heartbeat has to survive a
...
Two other readings of the disassembly do not hold:

- **`calctime` does not ride an arm of `0x439B80`.** `idle` is a script event the
  message loop dispatches; nothing in `0x439B80` dispatches it, and `0x43A860` is
  the ordinary world draw rather than "the idle branch".
- **`0x4397B0(2)` is not a delay.** It is four instructions that raise the redraw
  level at `0x48A630` to at least its argument — the flag `0x43A860` reads and
  clears at the top of every draw.

Answering it needs a **measurement of the original**, not more code: the
...
The port applies no multiplier: that would put a number in the endgame's clock
that no evidence supports, and the clock is what the whole mission is scored
against.
