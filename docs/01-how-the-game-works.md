...
*Titanic: Adventure Out of Time* is a first-person adventure game from 1996.
You explore the ship, look at things, pick up objects, and talk to people to
solve a mystery. The important thing to understand — because it shapes
*everything* about the file formats — is **how** it draws the world.

The ship is **not** a real-time 3D world. In 1996 no home computer could
...
|------|------------------|---------|
| **Set** | A room or section of the ship. One file. | The First-Class Lounge, cabin B59, the Grand Staircase |
| **Scene** | A *standpoint* inside a set — a spot you can stand on. | The middle of the lounge; just inside the cabin door |
| **View** | One *direction you can face* from a scene. | Facing the fireplace; facing the door |
...
closer to the camera. More on that in the [image codec doc](formats/image-codec.md).
...
first real screen. See **[BOOTFILE](formats/bootfile.md)**.
...
**[STG](formats/stg.md)** and **[SHP](formats/shp.md)**.
...
the strip below is the UI band. See **[SET](formats/set.md)** and the
**[image codec](formats/image-codec.md)**.
...
set"). Only if no script intercepts it does the default movement happen. See
**[Scripting](03-scripting-language.md)**.
...
**[MOV](formats/mov.md)** for close-ups and cutscenes.
...
switch. See **[Scripting](03-scripting-language.md)** and
...
few seconds. The engine has a heartbeat (~15 times a second) that services
these timed events. This is the "timing model" and is the current frontier of
the port.
...
**[the DFile container format](formats/README.md)**.
