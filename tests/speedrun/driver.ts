/**
 * The speedrun driver — the same gestures as tests/browser/driver.ts, in a hurry.
 *
 ...
 *
 * tests/browser/driver.ts is deliberately unhurried, and every bit of that is
...
// src/speedrun/driver.ts now, because the in-page runner needs exactly the same
...
export async function speedrunDriver(page: Page, opts: SpeedrunDriverOptions = {}): Promise<SpeedrunDriver & { page: Page; pagePoint(x: number, y: number): Promise<Point> }> {
  ...
  return {
     ...
     *
     * Shared with tests/playthrough/nav/aim.ts rather than reimplemented, for the
