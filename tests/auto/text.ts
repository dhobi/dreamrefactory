/**
 * The character set game text is stored in (src/df/text.ts), the per-language
 * table that says which one (src/languages.ts), and the line breaking that has
 * to cope with the result (src/fonts.ts).
 *
...
import { DEFAULT_ENCODING, decodeText, encodeText, sniffEncoding } from "../../src/df/text";
import { LANGUAGES, encodingOf } from "../../src/languages";
import { readPupFile } from "../../src/df/pup";
import { wrapText } from "../../src/fonts";
