 ...
 * src/df/mov.ts, so it runs without gamefiles/ — the editor must round-trip
...
import { FrameBuffer, decodeFrame, encodeFrame } from "../../src/df/image";
import {
  FLAG_KEY_SKIPS,
  MOV_NAME_FIELD,
  patchActionFrames,
...
import { buildMovFile } from "../../src/df/mov-build";
 ...
 * relative to its own header container (src/df/mov.ts). Deliberately given
