 ...
 *
 *   npx vitest run tests/auto/dust-saves.ts
 *
 ...
 *
 * Skipped, not failed, without the disc: `gamefiles/dust/save/` is a rip, and CI
 * has no rip (the same bargain `tests/auto/savegame.ts` makes for TAOOT).
 */
import { test, expect } from "vitest";
import { existsSync, readFileSync, readdirSync } from "node:fs";
import { join } from "node:path";
import { readSaveFile, writeSaveFile } from "../../src/df/savegame";
import { applyPatchV1, parseSaveV1 } from "../../src/df/savegame-v1";
import { shippedDustSaves } from "../../src/dust-saves";
...
test("the store finds the shipped saves in a manifest listing", () => {
  const found = shippedDustSaves([
    "gamefiles/dust/dustcd/DATA/G15.SET",
    "gamefiles/dust/save/START.RTD",
    "gamefiles/dust/save/DOG.RTD",
    // not ours: the disc's own files, and the play page's saves
    "gamefiles/dust/dustcd/INSTALL/ALT31/DF.EXE",
...
test("every shipped save is discoverable by the pattern the page uses", () => {
  const files = dustSaves();
  if (!files.length) return;
  const paths = files.map((f) => `gamefiles/dust/save/${f}`);
