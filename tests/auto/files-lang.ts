/**
 * The language axis of the browser file source (`src/files.ts`), and the shipped
 * saves that follow it (`src/save-seed.ts`).
 *
...
test("a manifest path names its disc and its language", () => {
  // the volumes come from the game's own setpath (src/engine/bootplan.ts), so the
...
test("shipped saves are picked from one language tree", () => {
  ...
  // served from (src/site.ts) — which under a test runner, with no document to
