 ...
 *
 *   npx vitest run tests/auto/regression.ts
...
// It used to be three hardcoded lists of TAOOT filenames in src/host.ts. Now the
...
// question GameHost.coldBoot asks, read from the BOOTFILE (src/engine/bootplan.ts)
const noDemo =
...
test("the deck map's jump table says what MAP.STG says", async () => {
  ...
  // The table is generated (tools/mapjumps.ts) from the same file the engine
...
// four seconds; a browser spends real ones (docs/verification.md).
