...
The version is `version` in `package.json` — **0.9.11**, semver, shown in the top
...
```
git switch -c release/0.9.11
npm version 0.9.11 --no-git-tag-version   # package.json + the lockfile
git commit -am "Version 0.9.11" && git push -u origin release/0.9.11
gh pr create --fill && gh pr merge --rebase --delete-branch   # once checks are green

git switch master && git pull
git tag v0.9.11 && git push --tags
```
