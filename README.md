...
The version is `version` in `package.json` — **0.9.5**, semver, shown in the top
...
```
git switch -c release/0.9.5
npm version 0.9.5 --no-git-tag-version   # package.json + the lockfile
git commit -am "Version 0.9.5" && git push -u origin release/0.9.5
