...
The version is `version` in `package.json` — **0.9.3**, semver, shown in the top
...
```
git switch -c release/0.9.3
npm version 0.9.3 --no-git-tag-version   # package.json + the lockfile
git commit -am "Version 0.9.3" && git push -u origin release/0.9.3
