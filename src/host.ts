...
import { AudioSink } from "./engine/audio";
import { GameSession } from "./engine/session";
import { FileProvider } from "./engine/setscripts";
import { BootPlan, EMPTY_BOOT_PLAN, readBootPlan } from "./engine/bootplan";
import { encodingOf } from "./languages";
 ...
 * and the manifest for everything else it needs (src/engine/bootplan.ts).
 *
...
export class GameHost {
   ...
   * that used to be hardcoded here (src/engine/bootplan.ts).
   *
   ...
   * (src/editions.ts), so there is no live invalidation to get wrong.
   */
  ...
  constructor(
    ...
    // file says which (src/df/text.ts) — the tree is the only thing that knows.
    // Asked live rather than copied, so a language switch cannot leave the
    // session decoding the one it used to be reading.
    this.session.textEncoding = () => encodingOf(files.activeEdition?.() ?? "");
  ...
  async coldBoot(): Promise<void> {
     ...
     * know (src/engine/bootplan.ts): whether this game's boot names a first room is
    ...
    if (!plan.landingSet) {
       ...
       * SetViewer, the frame loop draws when there is one (src/main.ts), and this
