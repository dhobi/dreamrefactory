 ...
 * edition; see src/editions.ts). Both follow the same shape: an active
 * selection, with a documented fallback.
 */
import { isEditionCode } from "./languages";
 ...
 * they are stated (src/engine/bootplan.ts) — this used to be a `/titanic([12])/`
...
export function editionOfUrl(url: string): string {
  ...
  return code && isEditionCode(code) ? code : NEUTRAL;
}

/**
 * Which of two URLs for the same basename ON THE SAME DISC wins. Mirrors
 * `preferred` in tools/gamefiles.ts so the page and the Node-side tools/tests
...
export class FileStore {
   ...
   * changes — what the page draws its busy mark from (src/main.ts).
   *
   ...
   * (public/lang.stg → src/lang-chooser.ts), or `?edition=`/localStorage
   * (src/editions.ts).
   *
