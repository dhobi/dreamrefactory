 ...
 * tools/mklangstg.ts) is an addition on top of the shipped data, not recovered
 ...
 * the Node-side index in tools/gamefiles.ts, the saved-game store — agrees here.
 *
...
export interface GameLanguage {
   ...
   * says — see src/df/text.ts for what was searched and what the original did
   * instead — so the tree it came from is the only thing left to ask, and this
   * is where the answer lives. tests/auto/text.ts re-derives every entry from
   * the shipped puppet files, so a wrong one here fails rather than mojibakes.
   */
  encoding: DfEncoding;
}

export const LANGUAGES: readonly GameLanguage[] = [
  { code: "en", label: "ENGLISH", name: "English", encoding: "macintosh" },
  { code: "de", label: "DEUTSCH", name: "Deutsch", encoding: "macintosh" },
  { code: "fr", label: "FRANÇAIS", name: "Français", encoding: "macintosh" },
  { code: "ru", label: "RUSSIAN", name: "Русский", encoding: "windows-1251" },
  { code: "nl", label: "NEDERLANDS", name: "Nederlands", encoding: "macintosh" },
  { code: "ja", label: "JAPANESE", name: "日本語", encoding: "shift_jis" },
];
 ...
 *    (src/lang-menu.ts), `?lang=`, {@link UI_LANG_STORAGE_KEY}. One of the six
 ...
 *    button row those three page groups carry (src/editions.ts), `?edition=`,
 ...
 */
export const LEGACY_EDITION_KEY = "taoot.lang";
 ...
 */
export const EXTRA_EDITIONS: readonly { code: string; name: string }[] = [
  { code: "demo", name: "Demo" },
];
