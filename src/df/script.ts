...
function pascalStringAt(data: Uint8Array, pos: number): string {
  const len = data[pos];
  let s = "";
  for (let i = 0; i < len; i++) s += String.fromCharCode(data[pos + 1 + i]);
  return s;
}
  ...
  for (;;) {
    ...
    switch (cmd) {
      case STRING: {
        ...
        tokens.push({ kind: "str", value: pascalStringAt(data, at) });
        break;
      }
      case VARIABLE: {
        const at = pos + info;
        if (at >= data.length) throw new Error("script: variable offset out of bounds");
        tokens.push({ kind: "var", name: pascalStringAt(data, at) });
        break;
      }
      case INTEGER:
  ...
  for (const t of tokens) {
    switch (t.kind) {
      ...
      case "break":
        ...
        break;
      case "op":
        if (t.id === 4018 || t.id === 8002) {
          // "(" and unary-capable "-" bind tightly: no trailing space
          out += t.name;
        } else if (t.id === 4019 || t.id === 4020) {
