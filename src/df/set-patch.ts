 ...
 * only reads sets (src/df/set.ts); everything that writes one back lives here.
 *
 ...
 * tests/auto/set-editor.ts). Frame art is replaced by the caller, which swaps a
 ...
 * writeContainerFile (src/df/container.ts) with only those bytes changed. False for a
