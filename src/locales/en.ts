 ...
 * (src/df/text.ts reads it, src/languages.ts says in which code page); nothing
 ...
 * language is English.
 *
 * The duplication is held together by tests/auto/locales.ts, which parses the
 ...
 * ours (src/locales/index.ts); ours is only to supply the forms, and the test
...
export const en = {
  ...
  site: {
    ...
    /** the label on the edition row — the play page, the editors and the
        collection all carry the same control (src/editions.ts), so they carry the
        same word for it */
   ...
   * "Une aventure hors du temps" — src/collection.ts's `RELEASE_TITLE` has the
  ...
  play: {
    ...
    // the two swipe options, which only a touch device is shown (src/main.ts).
   ...
   * DOS original itself under DBGL. `src/collection.ts` turns the box and
   ...
   * this page wrote — src/collection.ts reads them off src/languages.ts. The
   ...
   * and in src/collection.ts's own table, never in a catalogue.
   */
  collection: {
  ...
  casts: {
    ...
    posesInfoTail: "8 view directions per animation step" as Text,
    showThisPose: "show this pose" as Text,
    poseNameTitle: "pose name — what actorpose() asks for (max {max} characters)" as Text,
    manySteps: "several animation steps — a walk cycle the scheduler advances every 50 ms tick" as Text,
    oneStep: "a single step: a standing pose, shown from whichever of the 8 sides you see" as Text,
    byDirections: " × 8 directions · " as Text,
    ...
    gridHead: "“{name}” · rows are steps, columns are the 8 stored directions (0 = facing the viewer)" as Text,
  ...
  movies: {
    ...
    // what the PLAYER will do with this file, from the shared rule (src/df/mov-pace.ts):
 ...
 * and a test failure in tests/auto/locales.ts — never a blank label at runtime.
 */
