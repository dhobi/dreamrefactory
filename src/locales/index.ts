 ...
 * (src/lang-menu.ts) and by nothing else. What a page's CONTENT is read from — the
 * `gamefiles/` tree the game boots, the files an editor lists, the box the
 * collection turns — is the EDITION, `?edition=`, src/editions.ts. The two were
 ...
 */
import { LANGUAGES, DEFAULT_LANGUAGE, UI_LANG_STORAGE_KEY, isLanguageCode } from "../languages";
 ...
 * one directory; tests/auto/locales.ts fails if either shows up alone.
 */
...
export const translatedLanguages = (): string[] => [DEFAULT_LANGUAGE, ...Object.keys(CATALOGUES)];

/**
 * The language the *page* is in — see the order in the module comment. Only
 * ever one of {@link LANGUAGES}; anything else falls through to English.
 */
export function uiLanguage(): string {
  ...
  for (const code of wanted) {
    if (code && isLanguageCode(code) && translatedLanguages().includes(code.toLowerCase())) {
...
let activeCode: string = DEFAULT_LANGUAGE;
...
export const uiLanguageOptions = (): { code: string; name: string }[] =>
  LANGUAGES.filter((l) => translatedLanguages().includes(l.code)).map((l) => ({
    code: l.code,
    name: l.name,
  }));
