 ...
 * the shared reader.
 *
 * The saves live at `gamefiles/dust/save/*.RTD` and are listed in the page's own
 * manifest slice (`gamefiles-dust.json`), so seeding is the same trick the play
...
import { SaveEntry, SaveKind, getMeta, getSave, listSaves, putSave, setMeta } from "./save-store";
 ...
 *
 * Narrow and case-insensitive on purpose: `gamefiles/dust/save/` is the one
 * place these live (the disc itself is `gamefiles/dust/dustcd/`, and a `.RTD`
...
export function shippedDustSaves(manifestPaths: string[]): { rel: string; url: string; name: string }[] {
  ...
  for (const path of manifestPaths) {
    const m = /(?:^|\/)gamefiles\/dust\/save\/([^/]+\.rtd)$/i.exec(path);
    if (!m) continue;
    const base = m[1];
    found.push({
      rel: `${DISC}/${base}`,
 ...
 * (see `docs/formats/savegame.md`). Once a game has been loaded from a file that
