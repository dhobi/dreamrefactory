 ...
 * CONTENT is read from: src/editions.ts owns it, three page groups carry its
 ...
 * (src/locales/index.ts), the pages carry their English inline, and a reload is
 ...
 *
 * The authored chooser (public/lang.stg, src/lang-chooser.ts) is NOT this
 * control's other half any more: it runs before any tree has been read and what
 * it asks is which EDITION to boot.
 */
import { UI_LANG_STORAGE_KEY } from "./languages";
import { uiLanguage, uiLanguageOptions } from "./locales";
...
export function installLanguageMenu(): string {
  ...
  summary.textContent = `🌐 ${options.find((o) => o.code === code)?.name ?? code}`;
  // Hard-coded English, like every string this repo builds in TypeScript rather
  // than in markup (src/locales/en.ts says why); unlike most of those it is
